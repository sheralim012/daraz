...
@WebServlet(urlPatterns = {"/admin/products/manufacturer"})
        ...
        if (session != null) {
            ...
            if (role == (byte) 1) {
                ...
                if (query == null || query.isEmpty()) {
                    total = productDAO.countByManufacturer(manufacturerId);
                } else {
                    total = productDAO.countByManufacturerAndQuery(manufacturerId, query);
                }
                int offset = (currentPage - 1) * perPage;
                int totalPages = (int) Math.ceil((double) total / perPage);
                ArrayList<ProductBean> products = null;
                if (query == null || query.isEmpty()) {
                    products = productDAO.readLimitedByManufacturer(manufacturerId, perPage, offset);
                } else {
                    products = productDAO.readLimitedByManufacturerAndQuery(manufacturerId, query, perPage, offset);
                }
                ...
                getServletContext().getRequestDispatcher("/WEB-INF/admin/manufacturerProducts.jsp").forward(request, response);
            } else {
