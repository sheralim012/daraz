    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session != null) {
            ...
            if (role == (byte) 1) {
                ...
                long total = categoryDAO.countAll();
                int offset = (currentPage - 1) * perPage;
                int totalPages = (int) Math.ceil((double) total / perPage);
                ArrayList<CategoryBean> limitedCategories = categoryDAO.readLimited(perPage, offset);
                request.setAttribute("currentPage", currentPage);
                request.setAttribute("totalPages", totalPages);
                request.setAttribute("limitedCategories", limitedCategories);
                getServletContext().getRequestDispatcher("/WEB-INF/admin/manageCategories.jsp").forward(request, response);
            } else {
