...
@WebServlet(urlPatterns = {"/public/products/category"})
public class CategoryProductsController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (query == null || query.isEmpty()) {
            total = productDAO.countByCategoryWithVisibility((byte) 1, manufacturerId);
        } else {
            total = productDAO.countByCategoryAndQueryWithVisibility((byte) 1, manufacturerId, query);
        }
        int offset = (currentPage - 1) * perPage;
        int totalPages = (int) Math.ceil((double) total / perPage);
        ArrayList<ProductBean> products = null;
        if (query == null || query.isEmpty()) {
            products = productDAO.readLimitedByCategoryWithVisibility((byte) 1, manufacturerId, perPage, offset);
        } else {
            products = productDAO.readLimitedByCategoryAndQueryWithVisibility((byte) 1, manufacturerId, query, perPage, offset);
        }
        ...
        getServletContext().getRequestDispatcher("/WEB-INF/public/categoryProducts.jsp").forward(request, response);
    }
