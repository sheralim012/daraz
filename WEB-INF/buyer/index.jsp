            ...
            <ul class="pagination justify-content-center">
                <c:if test="${ totalPages gt 1 }">
                    <c:if test="${ currentPage - 1 ge 1 }">
                        <li class="page-item"><a class="page-link" href='<c:url value="/buyer/index?currentPage=${ currentPage - 1 }&query=${ query }" />'>Previous</a></li>
                    </c:if>
                    <c:forEach var="i" begin="1" end="${ totalPages }">
                        <c:choose>
                            <c:when test="${ i eq currentPage }">
                                <li class="page-item active"><a class="page-link" href='<c:url value="/buyer/index?currentPage=${ i }&query=${ query }" />'>${ i }</a></li>
                            </c:when>
                            <c:otherwise>
                                <li class="page-item"><a class="page-link" href='<c:url value="/buyer/index?currentPage=${ i }&query=${ query }" />'>${ i }</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${ (currentPage + 1) le totalPages }">
                        <li class="page-item"><a class="page-link" href='<c:url value="/buyer/index?currentPage=${ currentPage + 1 }&query=${ query }" />'>Next</a></li>
                    </c:if>
                </c:if>
            </ul>
        </nav>
